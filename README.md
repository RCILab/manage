...
- `003_degree_labels.sql` — 학위종류 명칭을 `석박통합과정/박사과정/석사과정/학사과정` 으로 통일 (002 를 이미 실행했다면 이것도 실행).
- `004_member_employed.sql` — 직장가입자 여부 컬럼 (+ 손재락·조민재 체크).
...
| `#/members` | 관리자 | 구성원 인사정보: 학위종류·학번·입학학기·생년월일·연구자번호·연락처·계좌번호·주민등록번호·BK·직장가입자·역할·활성 (+ 추가/수정 모달, CSV) |
